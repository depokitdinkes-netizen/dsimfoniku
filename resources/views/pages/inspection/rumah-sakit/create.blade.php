                    ...
                    <div class="form-control">
                        <label class="label cursor-pointer justify-start gap-2">
                            @php
                                ...
                                } elseif ($form_input['name'] == '9003' && isset($option['id']) && $option['id'] == '9003_a') {
                                    $isDefaultSelected = true; // First option for 9003
                                } elseif (in_array($form_input['name'], ['memiliki_alat_rontgen_portable', 'memiliki_shielding_radiasi']) && $option['value'] == 1) {
                                    $isDefaultSelected = true; // "Ya" option for these fields
                                } elseif (!in_array($form_input['name'], ['1002', '9003', 'memiliki_alat_rontgen_portable', 'memiliki_shielding_radiasi']) && $loop->first) {
                                    $isDefaultSelected = true; // First option for others
                                }
                            @endphp
                            <input type="radio" name="{{ $form_input['name'] }}" class="radio checked:bg-primary" value="{{ $actualValue }}" @if($isDefaultSelected) checked @endif />
